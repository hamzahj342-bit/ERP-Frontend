...
const DC_Listing = () => {
  ...
  const fetchDCs = async () => {
    try {
      const res = await api.get("/loader-documents", {
        params: { type: "DC", page: page, limit: 50 }
      });
  ...
  const handleDelete = async (id, no) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: `Do you want to delete DC No: ${no}?`,
      icon: 'warning',
        ...
        <div className="erp-page-card card">
          <div className="erp-page-header card-header header-flex">
            <div className="erp-page-header-left">
              ...
              <h2 className="erp-page-title">Delivery Challan (DC)</h2>
            </div>
            <button className="add-sale-btn erp-btn-primary" type="button" onClick={() => navigate('/dc-form')}>
              <FaPlus /> ADD NEW DC
            </button>
          </div>
                ...
                {documents.length > 0 ? (
                  documents.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td data-label="SALE" style={{ textAlign: 'center' }}>
                      ...
                      <td data-label="ACTION" className="erp-actions-cell">
                        <div className="erp-actions-group">
                          ...
                          <button type="button" onClick={() => navigate(`/dc-view/${d.no}`)} className="primary-btn">
                            <FaEye /> VIEW
                          </button>
                          ...
                          {!d.is_posted && (
                            <>
                              <button type="button" onClick={() => navigate(`/dc-form?editId=${d.id}`)} className="edit-btn-action">
                                <FaEdit /> EDIT
                              </button>
                  ...
                  <tr>
                    <td colSpan="7" style={{ textAlign: 'center', padding: '50px', color: '#94a3b8' }}>No DC Documents Found</td>
                  </tr>
                )}
              </tbody>
            </table>
